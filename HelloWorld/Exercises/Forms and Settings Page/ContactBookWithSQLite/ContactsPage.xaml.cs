...
namespace HelloWorld.Exercises.FormsandSettingsPage.ContactBookWithSQLite
{
    ...
    {
		private ObservableCollection<SQLiteContact> _contacts;
		...
		{
			await _connection.CreateTableAsync<SQLiteContact>();

			var contacts = await _connection.Table<SQLiteContact>().ToListAsync();

			_contacts = new ObservableCollection<SQLiteContact>(contacts);
			contactsListView.ItemsSource = _contacts;
		}

		async void OnAddContact(object sender, System.EventArgs e)
		{
			var page = new ContactDetailsPage(new SQLiteContact());
		...
		async void OnContactSelected(object sender, Xamarin.Forms.SelectedItemChangedEventArgs e)
		{
			...
			var selectedContact = e.SelectedItem as SQLiteContact;
		...
		{
			var contact = (sender as MenuItem).CommandParameter as SQLiteContact;
